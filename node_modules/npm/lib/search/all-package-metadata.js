...
// maintaining its metadata cache and making partials or full remote requests,
...
function createEntryUpdateStream (all, auth, staleness, latest, cb) {
  ...
  if (latest && (Date.now() - latest < (staleness * 1000))) {
    ...
    log.verbose('all-package-metadata', 'Cached data present with timestamp:', latest, 'requesting partials index update')
